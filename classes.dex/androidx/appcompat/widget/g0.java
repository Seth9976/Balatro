package androidx.appcompat.widget;

import android.app.SearchableInfo;
import android.content.ComponentName;
import android.content.Context;
import android.content.pm.ActivityInfo;
import android.content.pm.PackageManager.NameNotFoundException;
import android.content.pm.PackageManager;
import android.content.res.ColorStateList;
import android.content.res.Resources.NotFoundException;
import android.content.res.Resources;
import android.database.Cursor;
import android.graphics.drawable.Drawable.ConstantState;
import android.graphics.drawable.Drawable;
import android.net.Uri.Builder;
import android.net.Uri;
import android.os.Bundle;
import android.text.SpannableString;
import android.text.TextUtils;
import android.text.style.TextAppearanceSpan;
import android.util.Log;
import android.util.TypedValue;
import android.view.View.OnClickListener;
import android.view.View;
import android.view.ViewGroup;
import android.widget.ImageView;
import android.widget.TextView;
import c.e;
import java.io.FileNotFoundException;
import java.io.IOException;
import java.io.InputStream;
import java.util.List;
import java.util.WeakHashMap;
import q.c;

class g0 extends c implements View.OnClickListener {
    static final class a {
        public final TextView a;
        public final TextView b;
        public final ImageView c;
        public final ImageView d;
        public final ImageView e;

        public a(View view0) {
            this.a = (TextView)view0.findViewById(0x1020014);
            this.b = (TextView)view0.findViewById(0x1020015);
            this.c = (ImageView)view0.findViewById(0x1020007);
            this.d = (ImageView)view0.findViewById(0x1020008);
            this.e = (ImageView)view0.findViewById(e.k);
        }
    }

    private final SearchView l;
    private final SearchableInfo m;
    private final Context n;
    private final WeakHashMap o;
    private final int p;
    private boolean q;
    private int r;
    private ColorStateList s;
    private int t;
    private int u;
    private int v;
    private int w;
    private int x;
    private int y;

    public g0(Context context0, SearchView searchView0, SearchableInfo searchableInfo0, WeakHashMap weakHashMap0) {
        super(context0, searchView0.getSuggestionRowLayout(), null, true);
        this.q = false;
        this.r = 1;
        this.t = -1;
        this.u = -1;
        this.v = -1;
        this.w = -1;
        this.x = -1;
        this.y = -1;
        this.l = searchView0;
        this.m = searchableInfo0;
        this.p = searchView0.getSuggestionCommitIconResId();
        this.n = context0;
        this.o = weakHashMap0;
    }

    private void A(String s, Drawable drawable0) {
        if(drawable0 != null) {
            Drawable.ConstantState drawable$ConstantState0 = drawable0.getConstantState();
            this.o.put(s, drawable$ConstantState0);
        }
    }

    private void B(Cursor cursor0) {
        Bundle bundle0 = cursor0 == null ? null : cursor0.getExtras();
        if(bundle0 != null) {
            bundle0.getBoolean("in_progress");
        }
    }

    @Override  // q.a
    public CharSequence a(Cursor cursor0) {
        if(cursor0 == null) {
            return null;
        }
        CharSequence charSequence0 = g0.o(cursor0, "suggest_intent_query");
        if(charSequence0 != null) {
            return charSequence0;
        }
        if(this.m.shouldRewriteQueryFromData()) {
            CharSequence charSequence1 = g0.o(cursor0, "suggest_intent_data");
            if(charSequence1 != null) {
                return charSequence1;
            }
        }
        if(this.m.shouldRewriteQueryFromText()) {
            CharSequence charSequence2 = g0.o(cursor0, "suggest_text_1");
            return charSequence2 == null ? null : charSequence2;
        }
        return null;
    }

    @Override  // q.a
    public void b(Cursor cursor0) {
        if(this.q) {
            Log.w("SuggestionsAdapter", "Tried to change cursor after adapter was closed.");
            if(cursor0 != null) {
                cursor0.close();
            }
            return;
        }
        try {
            super.b(cursor0);
            if(cursor0 != null) {
                this.t = cursor0.getColumnIndex("suggest_text_1");
                this.u = cursor0.getColumnIndex("suggest_text_2");
                this.v = cursor0.getColumnIndex("suggest_text_2_url");
                this.w = cursor0.getColumnIndex("suggest_icon_1");
                this.x = cursor0.getColumnIndex("suggest_icon_2");
                this.y = cursor0.getColumnIndex("suggest_flags");
            }
        }
        catch(Exception exception0) {
            Log.e("SuggestionsAdapter", "error changing cursor and caching columns", exception0);
        }
    }

    @Override  // q.b$a
    public Cursor d(CharSequence charSequence0) {
        String s = charSequence0 == null ? "" : charSequence0.toString();
        if(this.l.getVisibility() == 0 && this.l.getWindowVisibility() == 0) {
            try {
                Cursor cursor0 = this.v(this.m, s, 50);
                if(cursor0 != null) {
                    cursor0.getCount();
                    return cursor0;
                }
            }
            catch(RuntimeException runtimeException0) {
                Log.w("SuggestionsAdapter", "Search suggestions query threw an exception.", runtimeException0);
            }
        }
        return null;
    }

    @Override  // q.a
    public void e(View view0, Context context0, Cursor cursor0) {
        a g0$a0 = (a)view0.getTag();
        int v = this.y == -1 ? 0 : cursor0.getInt(this.y);
        if(g0$a0.a != null) {
            String s = g0.w(cursor0, this.t);
            this.z(g0$a0.a, s);
        }
        if(g0$a0.b != null) {
            String s1 = g0.w(cursor0, this.v);
            CharSequence charSequence0 = s1 == null ? g0.w(cursor0, this.u) : this.l(s1);
            if(TextUtils.isEmpty(charSequence0)) {
                TextView textView0 = g0$a0.a;
                if(textView0 != null) {
                    textView0.setSingleLine(false);
                    g0$a0.a.setMaxLines(2);
                }
            }
            else {
                TextView textView1 = g0$a0.a;
                if(textView1 != null) {
                    textView1.setSingleLine(true);
                    g0$a0.a.setMaxLines(1);
                }
            }
            this.z(g0$a0.b, charSequence0);
        }
        ImageView imageView0 = g0$a0.c;
        if(imageView0 != null) {
            this.y(imageView0, this.t(cursor0), 4);
        }
        ImageView imageView1 = g0$a0.d;
        if(imageView1 != null) {
            this.y(imageView1, this.u(cursor0), 8);
        }
        if(this.r != 2 && (this.r != 1 || (v & 1) == 0)) {
            g0$a0.e.setVisibility(8);
            return;
        }
        g0$a0.e.setVisibility(0);
        CharSequence charSequence1 = g0$a0.a.getText();
        g0$a0.e.setTag(charSequence1);
        g0$a0.e.setOnClickListener(this);
    }

    @Override  // q.a
    public View getDropDownView(int v, View view0, ViewGroup viewGroup0) {
        try {
            return super.getDropDownView(v, view0, viewGroup0);
        }
        catch(RuntimeException runtimeException0) {
            Log.w("SuggestionsAdapter", "Search suggestions cursor threw exception.", runtimeException0);
            View view1 = this.g(this.n, this.c(), viewGroup0);
            if(view1 != null) {
                ((a)view1.getTag()).a.setText(runtimeException0.toString());
            }
            return view1;
        }
    }

    @Override  // q.a
    public View getView(int v, View view0, ViewGroup viewGroup0) {
        try {
            return super.getView(v, view0, viewGroup0);
        }
        catch(RuntimeException runtimeException0) {
            Log.w("SuggestionsAdapter", "Search suggestions cursor threw exception.", runtimeException0);
            View view1 = this.h(this.n, this.c(), viewGroup0);
            if(view1 != null) {
                ((a)view1.getTag()).a.setText(runtimeException0.toString());
            }
            return view1;
        }
    }

    @Override  // q.c
    public View h(Context context0, Cursor cursor0, ViewGroup viewGroup0) {
        View view0 = super.h(context0, cursor0, viewGroup0);
        view0.setTag(new a(view0));
        ((ImageView)view0.findViewById(e.k)).setImageResource(this.p);
        return view0;
    }

    @Override  // android.widget.BaseAdapter
    public boolean hasStableIds() {
        return false;
    }

    private Drawable k(String s) {
        Drawable.ConstantState drawable$ConstantState0 = (Drawable.ConstantState)this.o.get(s);
        return drawable$ConstantState0 == null ? null : drawable$ConstantState0.newDrawable();
    }

    private CharSequence l(CharSequence charSequence0) {
        if(this.s == null) {
            TypedValue typedValue0 = new TypedValue();
            this.n.getTheme().resolveAttribute(c.a.q, typedValue0, true);
            this.s = this.n.getResources().getColorStateList(typedValue0.resourceId);
        }
        CharSequence charSequence1 = new SpannableString(charSequence0);
        ((SpannableString)charSequence1).setSpan(new TextAppearanceSpan(null, 0, 0, this.s, null), 0, charSequence0.length(), 33);
        return charSequence1;
    }

    private Drawable m(ComponentName componentName0) {
        String s;
        ActivityInfo activityInfo0;
        PackageManager packageManager0 = this.n.getPackageManager();
        try {
            activityInfo0 = packageManager0.getActivityInfo(componentName0, 0x80);
        }
        catch(PackageManager.NameNotFoundException packageManager$NameNotFoundException0) {
            s = packageManager$NameNotFoundException0.toString();
            Log.w("SuggestionsAdapter", s);
            return null;
        }
        int v = activityInfo0.getIconResource();
        if(v == 0) {
            return null;
        }
        Drawable drawable0 = packageManager0.getDrawable(componentName0.getPackageName(), v, activityInfo0.applicationInfo);
        if(drawable0 == null) {
            s = "Invalid icon resource " + v + " for " + componentName0.flattenToShortString();
            Log.w("SuggestionsAdapter", s);
            return null;
        }
        return drawable0;
    }

    private Drawable n(ComponentName componentName0) {
        String s = componentName0.flattenToShortString();
        Drawable drawable0 = null;
        if(this.o.containsKey(s)) {
            Drawable.ConstantState drawable$ConstantState0 = (Drawable.ConstantState)this.o.get(s);
            return drawable$ConstantState0 == null ? null : drawable$ConstantState0.newDrawable(this.n.getResources());
        }
        Drawable drawable1 = this.m(componentName0);
        if(drawable1 != null) {
            drawable0 = drawable1.getConstantState();
        }
        this.o.put(s, drawable0);
        return drawable1;
    }

    @Override  // android.widget.BaseAdapter
    public void notifyDataSetChanged() {
        super.notifyDataSetChanged();
        this.B(this.c());
    }

    @Override  // android.widget.BaseAdapter
    public void notifyDataSetInvalidated() {
        super.notifyDataSetInvalidated();
        this.B(this.c());
    }

    public static String o(Cursor cursor0, String s) {
        return g0.w(cursor0, cursor0.getColumnIndex(s));
    }

    @Override  // android.view.View$OnClickListener
    public void onClick(View view0) {
        Object object0 = view0.getTag();
        if(object0 instanceof CharSequence) {
            this.l.R(((CharSequence)object0));
        }
    }

    private Drawable p() {
        Drawable drawable0 = this.n(this.m.getSearchActivity());
        return drawable0 == null ? this.n.getPackageManager().getDefaultActivityIcon() : drawable0;
    }

    private Drawable q(Uri uri0) {
        try {
            if(!"android.resource".equals(uri0.getScheme())) {
                InputStream inputStream0 = this.n.getContentResolver().openInputStream(uri0);
                if(inputStream0 == null) {
                    throw new FileNotFoundException("Failed to open " + uri0);
                }
                try {
                    return Drawable.createFromStream(inputStream0, null);
                }
                finally {
                    try {
                        inputStream0.close();
                    }
                    catch(IOException iOException0) {
                        Log.e("SuggestionsAdapter", "Error closing icon stream for " + uri0, iOException0);
                    }
                }
            }
            try {
                return this.r(uri0);
            }
            catch(Resources.NotFoundException unused_ex) {
                throw new FileNotFoundException("Resource does not exist: " + uri0);
            }
        }
        catch(FileNotFoundException fileNotFoundException0) {
            Log.w("SuggestionsAdapter", "Icon not found: " + uri0 + ", " + fileNotFoundException0.getMessage());
            return null;
        }
    }

    Drawable r(Uri uri0) {
        int v1;
        Resources resources0;
        String s = uri0.getAuthority();
        if(TextUtils.isEmpty(s)) {
            throw new FileNotFoundException("No authority: " + uri0);
        }
        try {
            resources0 = this.n.getPackageManager().getResourcesForApplication(s);
        }
        catch(PackageManager.NameNotFoundException unused_ex) {
            throw new FileNotFoundException("No package found for authority: " + uri0);
        }
        List list0 = uri0.getPathSegments();
        if(list0 == null) {
            throw new FileNotFoundException("No path: " + uri0);
        }
        int v = list0.size();
        if(v == 1) {
            try {
                v1 = Integer.parseInt(((String)list0.get(0)));
            }
            catch(NumberFormatException unused_ex) {
                throw new FileNotFoundException("Single path segment is not a resource ID: " + uri0);
            }
        }
        else {
            if(v != 2) {
                throw new FileNotFoundException("More than two path segments: " + uri0);
            }
            v1 = resources0.getIdentifier(((String)list0.get(1)), ((String)list0.get(0)), s);
        }
        if(v1 == 0) {
            throw new FileNotFoundException("No resource found for: " + uri0);
        }
        return resources0.getDrawable(v1);
    }

    private Drawable s(String s) {
        if(s != null && !s.isEmpty() && !"0".equals(s)) {
            try {
                int v = Integer.parseInt(s);
                Drawable drawable0 = this.k("android.resource://com.playstack.balatro.android/" + v);
                if(drawable0 != null) {
                    return drawable0;
                }
                Drawable drawable1 = androidx.core.content.a.c(this.n, v);
                this.A("android.resource://com.playstack.balatro.android/" + v, drawable1);
                return drawable1;
            }
            catch(NumberFormatException unused_ex) {
                Drawable drawable2 = this.k(s);
                if(drawable2 != null) {
                    return drawable2;
                }
                Drawable drawable3 = this.q(Uri.parse(s));
                this.A(s, drawable3);
                return drawable3;
            }
            catch(Resources.NotFoundException unused_ex) {
                Log.w("SuggestionsAdapter", "Icon resource not found: " + s);
                return null;
            }
        }
        return null;
    }

    private Drawable t(Cursor cursor0) {
        int v = this.w;
        if(v == -1) {
            return null;
        }
        Drawable drawable0 = this.s(cursor0.getString(v));
        return drawable0 == null ? this.p() : drawable0;
    }

    private Drawable u(Cursor cursor0) {
        return this.x == -1 ? null : this.s(cursor0.getString(this.x));
    }

    Cursor v(SearchableInfo searchableInfo0, String s, int v) {
        String[] arr_s = null;
        if(searchableInfo0 == null) {
            return null;
        }
        String s1 = searchableInfo0.getSuggestAuthority();
        if(s1 == null) {
            return null;
        }
        Uri.Builder uri$Builder0 = new Uri.Builder().scheme("content").authority(s1).query("").fragment("");
        String s2 = searchableInfo0.getSuggestPath();
        if(s2 != null) {
            uri$Builder0.appendEncodedPath(s2);
        }
        uri$Builder0.appendPath("search_suggest_query");
        String s3 = searchableInfo0.getSuggestSelection();
        if(s3 == null) {
            uri$Builder0.appendPath(s);
        }
        else {
            arr_s = new String[]{s};
        }
        if(v > 0) {
            uri$Builder0.appendQueryParameter("limit", String.valueOf(v));
        }
        Uri uri0 = uri$Builder0.build();
        return this.n.getContentResolver().query(uri0, null, s3, arr_s, null);
    }

    private static String w(Cursor cursor0, int v) {
        if(v == -1) {
            return null;
        }
        try {
            return cursor0.getString(v);
        }
        catch(Exception exception0) {
            Log.e("SuggestionsAdapter", "unexpected error retrieving valid column from cursor, did the remote process die?", exception0);
            return null;
        }
    }

    public void x(int v) {
        this.r = v;
    }

    private void y(ImageView imageView0, Drawable drawable0, int v) {
        imageView0.setImageDrawable(drawable0);
        if(drawable0 == null) {
            imageView0.setVisibility(v);
            return;
        }
        imageView0.setVisibility(0);
        drawable0.setVisible(false, false);
        drawable0.setVisible(true, false);
    }

    private void z(TextView textView0, CharSequence charSequence0) {
        textView0.setText(charSequence0);
        textView0.setVisibility((TextUtils.isEmpty(charSequence0) ? 8 : 0));
    }
}

